function patternChiseled() {
    ...
    for (let x = 0; x < columnCount; x++) {
      ...
      for (let y = 0; y < rowCount; y++) {
        ...
        if (Math.random() < density) {
          ...
          var shapeSelection = getRandomInt(2, 4);
          ...
          // If 2 draw split
            ...
            document.getElementById('svg').innerHTML += split(x, y, chosenPalette, paddingOffset);
          }
          
          // If 3 draw squareInSquare
          else if (shapeSelection === 3) {
            document.getElementById('svg').innerHTML += squareInSquare(x, y, chosenPalette, paddingOffset);
          }
          
          // If 4 draw squareInSplit
          else if (shapeSelection === 4) {
            document.getElementById('svg').innerHTML += squareInSplit(x, y, chosenPalette, paddingOffset);
          }
  ...
  function square(x, y, chosenPalette, paddingOffset) {
    ...
    const path = '<rect transform="'+transform+'" fill="'+sample(chosenPalette.colors)+'" width="60" height="60"/>'
    return '<svg>' + path + '</svg>';
  }

  function split(x, y, chosenPalette, paddingOffset) {
    ...
    const transform = 'translate(' + ((x * 60) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ')';
    const path =  '<rect transform="'+transform+'" fill="'+color1+'" width="60" height="60"/>'+
                  '<path transform="'+transform+' translate(30,30) rotate(' + rotate + ') translate(-30,-30)" fill="'+color2+'" d="M60 0H0v60L60 0z"/>'
    return '<svg>' + path + '</svg>';
  }

  function squareInSquare(x, y, chosenPalette, paddingOffset) {
    var color1 = sample(chosenPalette.colors);
    var color2 = sample(chosenPalette.colors);
    while (color1 === color2) {
      var color2 = sample(chosenPalette.colors);
    ...
    const transform = 'translate(' + ((x * 60) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ')';
    const path =  '<rect transform="'+transform+'" fill="'+color1+'" width="60" height="60"/>'+
                  '<rect transform="'+transform+'" fill="'+color2+'" x="15" y="15" width="30" height="30"/>'
    return '<svg>' + path + '</svg>';
  }

  function squareInSplit(x, y, chosenPalette, paddingOffset) {
    var color1 = sample(chosenPalette.colors);
    var color2 = sample(chosenPalette.colors);
    while (color1 === color2) {
      var color2 = sample(chosenPalette.colors);
    }
    var color3 = sample(chosenPalette.colors);
    ...
    const path =  '<rect transform="'+transform+'" fill="'+color1+'" width="60" height="60"/>'+
                  '<path transform="'+transform+' translate(30,30) rotate(' + rotate + ') translate(-30,-30)" fill="'+color2+'" d="M60 0H0v60L60 0z"/>'+
                  '<rect transform="'+transform+'" fill="'+color3+'" x="15" y="15" width="30" height="30"/>'
    return '<svg>' + path + '</svg>';
  }
