function patternRiseAndSet() {
    ...
    for (let x = 0; x < columnCount; x++) {
      ...
      for (let y = 0; y < rowCount; y++) {
        ...
        if (Math.random() < density) {
          ...
          // If row is even (remember zero) draw sunrise
          if (y & 1) {
            document.getElementById('svg').innerHTML += set(x, y, chosenPalette, paddingOffset);
          }

          // If row is odd
          else {
            document.getElementById('svg').innerHTML += rise(x, y, chosenPalette, paddingOffset);
          }
        }
  ...
  function mask(columnCount, rowCount, paddingOffset) {
    ...
    const path = '<rect transform="'+transform+'" width="'+((columnCount*60)*2)+'" height="'+(rowCount*60)+'" fill="white" />'
    return  '<mask id="mask" mask-type="alpha" maskUnits="userSpaceOnUse">' + 
                path + 
              '</mask>'
  }

  function rise(x, y, chosenPalette, paddingOffset) {
    const transform = 'translate(' + ((x * 120) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ')';
    const path = '<path transform="' + transform + '" d="M120 60c0-33.137-26.863-60-60-60S0 26.863 0 60h120z" fill="' + sample(chosenPalette.colors) + '"/>'
    return '<svg>' + path + '</svg>'
  }

  function set(x, y, chosenPalette, paddingOffset) {
    const transform = 'translate(' + ((x * 120) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ')';
    const path = '<path transform="' + transform + '" d="M0 0c0 33.137 26.863 60 60 60s60-26.863 60-60H0z" fill="' + sample(chosenPalette.colors) + '"/>'
    return '<svg>' + path + '</svg>'
  }
