function patternStigma() {
      ...
      for (let y = 0; y < rowCount; y++) {
        ...
        if (Math.random() < density) {
          ...
          // If 1 draw square
            ...
            document.getElementById('svg').innerHTML += oneRounded(x, y, chosenPalette, paddingOffset);
          }
          
          // If 2 draw circle
          else if (shapeSelection === 2) {
            document.getElementById('svg').innerHTML += twoRounded(x, y, chosenPalette, paddingOffset);
          }
          
          // If 3 draw Quarter Circle inside of a Square
          else if (shapeSelection === 3) {
            document.getElementById('svg').innerHTML += threeRounded(x, y, chosenPalette, paddingOffset);
          }
  ...
  function oneRounded(x, y, chosenPalette, paddingOffset) {
    ...
    const path = '<path transform="' + transform + '" d="M0 0h30c16.569 0 30 13.431 30 30v30H0V0z" fill="' + sample(chosenPalette.colors) + '"/>'
    return '<svg>' + path + '</svg>';
  }

  function twoRounded(x, y, chosenPalette, paddingOffset) {
    const rotate = 90 * getRandomInt(0,3);
    const transform = 'translate(' + ((x * 60) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ') translate(30,30) rotate(' + rotate + ') translate(-30,-30)';
    const path = '<path transform="' + transform + '" d="M0 0h30c16.569 0 30 13.431 30 30 0 16.569-13.431 30-30 30H0V0z" fill="' + sample(chosenPalette.colors) + '"/>'
    return '<svg>' + path + '</svg>';
  }

  function threeRounded(x, y, chosenPalette, paddingOffset) {
    const rotate = 90 * getRandomInt(0,3);
    const transform = 'translate(' + ((x * 60) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ') translate(30,30) rotate(' + rotate + ') translate(-30,-30)';
    const path = '<path transform="' + transform + '" d="M0 0h30c16.569 0 30 13.431 30 30 0 16.569-13.431 30-30 30C13.431 60 0 46.569 0 30V0z" fill="' + sample(chosenPalette.colors) + '"/>'
    return '<svg>' + path + '</svg>';
  }

  function circle(x, y, chosenPalette, paddingOffset) {
    const transform = 'translate(' + ((x * 60) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ')';
    const path = '<path transform="' + transform + '" d="M0 30C0 13.431 13.431 0 30 0c16.569 0 30 13.431 30 30 0 16.569-13.431 30-30 30C13.431 60 0 46.569 0 30z" fill="' + sample(chosenPalette.colors) + '"/>'
    return '<svg>' + path + '</svg>';
  }
}