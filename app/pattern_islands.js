function patternIslands() {
    ...
    for (let x = 0; x < columnCount; x++) {
      ...
      for (let y = 0; y < rowCount; y++) {
        ...
        if (Math.random() < density) {
          document.getElementById('svg').innerHTML += islands(x, y, chosenPalette, paddingOffset, color1, color2);
        }
  ...
  function islands(x, y, chosenPalette, paddingOffset, color1, color2) {
    ...
    const path =  '<rect transform="'+transform+'" fill="'+colorA+'" width="60" height="60"/>'+
                  '<path transform="'+transform+' translate(30,30) rotate(' + rotate + ') translate(-30,-30)" fill="'+colorB+'" d="M60 0H40v20H20v20H0v20h60V0z"/>'
    return '<svg>' + path + '</svg>';
  }
}