function patternStairs() {
    ...
    for (let x = 0; x < columnCount; x++) {
      ...
      for (let y = 0; y < rowCount; y++) {
        ...
        if (Math.random() < density) {
          document.getElementById('svg').innerHTML += stairs(x, y, chosenPalette, paddingOffset);
        }
  ...
  function stairs(x, y, chosenPalette, paddingOffset) {
    ...
    const path =  '<rect transform="'+transform+'" fill="'+color1+'" width="60" height="60"/>'+
                  '<path transform="'+transform+' translate(30,30) rotate(' + rotate + ') translate(-30,-30)" fill="'+color2+'" d="M60 0H40v20H20v20H0v20h60V0z"/>'
    return '<svg>' + path + '</svg>';
  }
}