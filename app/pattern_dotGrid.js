function patternDotGrid() {
    ...
    for (let x = 0; x < columnCount; x++) {
      ...
      for (let y = 0; y < rowCount; y++) {
        ...
        if (Math.random() < density) {
          document.getElementById('svg').innerHTML += dot(x, y, chosenPalette, paddingOffset);
        }
  ...
  function dot(x, y, chosenPalette, paddingOffset) {
    const transform = 'translate(' + ((x * 60) + paddingOffset) + ', ' + ((y * 60) + paddingOffset) + ')';
    const path = '<circle fill="'+sample(chosenPalette.colors)+'" transform="'+transform+'" cx="30" cy="30" r="15"/>'
    return '<svg>' + path + '</svg>'
  }
}